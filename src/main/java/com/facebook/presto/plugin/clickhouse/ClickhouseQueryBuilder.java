...
public class ClickhouseQueryBuilder
    ...
    public PreparedStatement buildSql(
        ...
        if (additionalPredicate.isPresent()) {
            ...
            accumulator.addAll(additionalPredicate.get().getBoundConstantValues().stream()
                    .map(constantExpression -> new ClickhouseQueryBuilder.TypeAndValue(constantExpression.getType(), constantExpression.getValue()))
        ...
        for (int i = 0; i < accumulator.size(); i++) {
            ClickhouseQueryBuilder.TypeAndValue typeAndValue = accumulator.get(i);
    ...
    private static void bindValue(Object value, Type type, List<TypeAndValue> accumulator) {
        ...
        accumulator.add(new ClickhouseQueryBuilder.TypeAndValue(type, value));
    }
}